...
drive_dist_err_std = 0.04                   # Standard deviation of error in distance when driving
drive_ang_err_mean = 0.0                    # Mean of error in angle when driving
drive_ang_err_std = 0.01                    # Standard deviation of error in angle when driving
rot_dist_err_mean = 0.0                     # Mean of error in distance when rotating
rot_dist_err_std = 0.005                    # Standard deviation of error in distance when rotating
rot_ang_err_mean = 0.0                      # Mean of error in angle when rotating
rot_ang_err_std = 0.01                      # Standard deviation of error in angle when rotating

# ODOMETRY SIM ERRORS  (odometry errors, added to the actual position - errors in angle measurement)
sim_drive_ang_err_std = 0.07                # This gets multiplied by real meters driven
sim_rot_ang_err_std = 0.05                  # This gets multiplied by rads of real rotation

# INDICES FOR SENSOR SIMULATION:
wd1 = 0.0063
wd0 = -0.0005
wa6 = 115.4385
wa4 = 0.15973
...
def process_scan(distances, wall_angles):
    const = 0.0 # 0.008
    # Get weights
    wd = get_weights_distance(distances)                            # Get weights for all simulated scan distances only once
    wa = get_weights_angle(wall_angles)                             # Get weights for all simulated wall angles only once
    wi = get_weights_index(61)                                      # Get weights for all indices only once
    ...
    for i in range(91):
        ...
        dist_sum = 0.0                                              # Sum of distances, multiplied by their weights
        ...
        for j in range(-30, 31, 1):
            if distances[(I + j) % 720] is not INFINITY and wall_angles[(I + j) % 720] is not INFINITY:
                w = wd[(I + j) % 720] * wa[(I + j) % 720] * wi[j + 30]      # Multiply all the weights together
                w_sum += w
                dist_sum += distances[(I + j) % 720] * w
        # If the final w_sum is too small, it is discarded (as if the sensor would not sense such a weak signal)
        if w_sum >= 0.015:
            sim_distances[i] = (dist_sum / w_sum) + const
            noise_std = 0.002 / w_sum + 0.001                       # Weaker the signal, stronger the noise
            noise = np.random.normal(0.0, noise_std)
            sim_distances[i] += noise
        if sim_distances[i] > 1.35:                                 # Set every distance, larger than 1.35 to 0 (sensor does not publish distances that large)
            sim_distances[i] = 0.0
...
# Calculates and returns the weights for distances
def get_weights_distance(distances):
    distance_weights = np.empty(distances.shape)
    for i in range(distances.shape[0]):
        distance_weights[i] = wd1 * distances[i]**(-2) + wd0
        distance_weights[i] = min(distance_weights[i], 1.0)
        distance_weights[i] = max(distance_weights[i], 0.001)

    return distance_weights


# Calculates and returns the weights for wall_angles
def get_weights_angle(angles):
    angles = abs((pi / 2.0) - angles)
    angle_weights = np.zeros(angles.shape)
    for i in range(angles.shape[0]):
        angle_weights[i] = ((0.33333 * exp(-(angles[i]**6 / wa6))) + (0.33333 * exp(-(angles[i]**4 / wa4))) + (0.33333 * exp(-angles[i]**2 / wa2)) + wa0) * 2.0
        angle_weights[i] = min(angle_weights[i], 1.0)
        angle_weights[i] = max(angle_weights[i], 0.0)

    return angle_weights


# Calculates and returns the weigthts for indices
def get_weights_index(n):
    indices = np.empty((n))
    index_weights = np.empty((n))
    n_2 = floor(n / 2.0)

    for i in range(n):
        indices[i] = np.abs(n_2 - i)
        index_weights[i] = ((wi6s * exp(-(pow(indices[i], 6) / wi6))) + (wi4s * exp(-(pow(indices[i], 4) / wi4))) + (wi2s * exp(-(pow(indices[i], 2) / wi2)))) * 0.8
        index_weights[i] = min(index_weights[i], 1.0)
        index_weights[i] = max(index_weights[i], 0.0)
